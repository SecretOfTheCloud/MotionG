...
class InverseKinematics(object):
    def __init__(self, positions, joints_name, parent_list, children_list, initial_offset, bvh_info):
        ...
        self.down_neck = bvh_info["down_neck_index"]
        self.up_neck = bvh_info["up_neck_index"]
        self.left_shoulder = bvh_info["left_shoulder_index"]
        self.right_shoulder = bvh_info["right_shoulder_index"]
    ...
    def calculate_rotation_for_frame(self, frame):
        rotations = [self.positions[frame][0] - self.offsets[0]]
        ...
        for i, name in enumerate(self.joints):
            ...
            elif i == self.down_neck:
                neck_rotation = self.get_neck_rotation(frame, rotations)
                rotations[i + 1] = neck_rotation
    ...
    def get_neck_rotation(self, frame, rotations):
        neck_init = self.offsets[self.up_neck]

        # get goal
        vec = self.positions[frame][self.up_neck] - self.positions[frame][self.down_neck]
        temp_goal = []
        parent_list = []
        now = self.down_neck
        while self.parents[now] != -1:
        ...
        neck_euler = self.calculate_rotation_for_node(neck_init, neck_goal)

        quaternion1 = euler_to_quaternion(neck_euler, self.rotation_order)
        rotate_axis = neck_goal

        rotate_point = self.positions[frame][self.down_neck]
        euler = self.refine_neck_euler(frame, quaternion1, rotate_axis, rotate_point, rotations)

        return euler

    def refine_neck_euler(self, frame, quaternion1, rotate_axis, rotate_point, rotations):
        ...
        def calculate_error(rot_angle):
            ...
            rotations[self.down_neck + 1] = _euler
            self.update_children(rotations, frame)

            _error = np.sum(abs(self.positions[frame][self.left_shoulder] -
                                self.changed_positions[frame][self.left_shoulder]) + \
                            abs(self.positions[frame][self.right_shoulder] -
                                self.changed_positions[frame][self.right_shoulder]), axis=-1)
            return _error, _euler
