...
def main():
    ...
    for i, item in enumerate(bvh_files):
        ...
        input_frame, output_frame, input_clip = process_data(item, start=start, end=stop)
...
def process_data(bvh_file, start=None, end=None, ordre=None, network = None):
    ...
    v=x[:,108:192]
    longeur_x = len(x)
    v_p = np.array([]).reshape((0, 10164))
    for index in range(60, longeur_x-60):
        features = []
        for joint in range(84):
            for modification in range(-60,61):
                features.append(v[index+modification,joint])
        v_p=np.append(v_p,np.array(features).reshape(1,-1),axis=0)

    print(v_p.shape)
    print(v_p[1:5,0:15])

    np.savez_compressed('../datasets/P/v_p.npz', vp=v_p)
